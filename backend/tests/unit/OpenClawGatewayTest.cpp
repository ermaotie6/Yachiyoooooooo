...
TEST_F(OpenClawGatewayTest, Initialize_Default) {
    ...
    bool result = gateway->initialize("http://localhost:8765", "", "deepseek/deepseek-v4-flash", 30);
    EXPECT_TRUE(result);
}

TEST_F(OpenClawGatewayTest, Initialize_CustomEndpoint) {
    bool result = gateway->initialize("http://192.168.1.100:9999", "", "deepseek/deepseek-v4-flash", 10);
    EXPECT_TRUE(result);
}
...
TEST_F(OpenClawGatewayTest, IsHealthy_NoService) {
    gateway->initialize("http://localhost:99999", "", "deepseek/deepseek-v4-flash", 2);
...
TEST_F(OpenClawGatewayTest, ClearCache_Empty) {
    gateway->initialize("http://localhost:8765");
    gateway->clearCache();
    // 清空空缓存不应崩溃
}
